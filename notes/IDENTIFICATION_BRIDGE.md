...
In-paper anchor: `paper/LEOPOLDT_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the expected `p`-adic regulator rank through an admissible regulator closure architecture`.
...
- endpoint lock: `regulator_transfer`.
...
2. the endpoint-transfer lock `regulator_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_LEO` remains positive.
...
Payload: verify `regulator_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_LEO` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `regulator_transfer`
