...
In-paper anchor: `paper/LEOPOLDT_CONJECTURE_PREPRINT.md` (`LEO_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the expected `p`-adic regulator rank through an admissible regulator closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`LEO_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and regulator windows do not collapse.
This is the first-failure capture contribution to `M_LEO`.
...
If near-failure extraction and no-Zeno spacing hold, then `LEO_G3` prevents hidden drift before rigidity is applied.
